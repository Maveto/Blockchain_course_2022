...
contract TestAmazon {
    ...
    event clientBuy(string name, address wallet, uint256 quantity);
    mapping(uint256 => Product) public listProducts;
    ...
    function addProduct(Product memory productData) public onlyOwner closedAmazon {
        require(
            bytes(productData.name).length > 5,
            "The name of product should be more than 5."
        );
        listProducts[productData.id] = Product(
            productData.id,
            productData.name,
            0,
            productData.price
     ...
     */
    function addQuantity(uint256 id, uint256 stock) public onlyOwner closedAmazon {
     ...
     */
    function buyProduct(uint256 id, uint256 quantity) public payable closedAmazon {
        Product memory productData = listProducts[id];
        require(
            (quantity * productData.price) <= convertWeiToEther(msg.value),
            "You don't have enought money."
        );
        require(
            quantity <= productData.stock,
            "There is not the stock in product."
        );
     ...
     */
    function convertWeiToEther(uint256 amountInWei) private pure returns (uint256) {
